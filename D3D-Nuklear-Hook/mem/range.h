...
class range {
  public:
     ...
     */
    range(address_t base, std::size_t size) noexcept : m_base(base), m_size(size) {}
    ...
    [[nodiscard]] address_t scan(const pattern& sig) const noexcept {
        ...
        for (auto current_page = m_base.cast<const BYTE*>();
             ...
             current_page = reinterpret_cast<const BYTE*>(
                 reinterpret_cast<uintptr_t>(page_info.BaseAddress) + page_info.RegionSize)) {
            if (!is_page_valid(page_info, current_page)) {
                continue;
            }

            const auto* region_end =
                static_cast<const BYTE*>(page_info.BaseAddress) + page_info.RegionSize;
    ...
    [[nodiscard]] std::vector<address_t> scan_all(const pattern& sig) const {
        ...
        for (auto current_page = m_base.cast<const BYTE*>();
            ...
            scan_page(static_cast<const BYTE*>(page_info.BaseAddress), page_info.RegionSize, sig,
                      results, false);
        }
        return results;
    }
     ...
     * @return Address of the first match, or null address if not found
     */
    [[nodiscard]] address_t scan_range(const pattern& sig, std::size_t offset,
                                       std::size_t scan_range) const noexcept {
  ...
  private:
     ...
     */
    [[nodiscard]] static bool is_page_valid(MEMORY_BASIC_INFORMATION& page_info,
                                            const BYTE* current_page) noexcept {
        ...
        return page_info.State == MEM_COMMIT &&
               !(page_info.Protect &
                 (PAGE_NOACCESS | PAGE_GUARD | PAGE_WRITECOPY | PAGE_EXECUTE_WRITECOPY));
    }
     ...
     */
    [[nodiscard]] static bool pattern_matches(const uint8_t* target, const unsigned char* bytes,
                                              const uint8_t* masks, std::size_t length) noexcept {
