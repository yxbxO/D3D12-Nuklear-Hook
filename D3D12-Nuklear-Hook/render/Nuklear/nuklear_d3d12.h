...
nk_d3d12_render(ID3D12GraphicsCommandList* command_list, enum nk_anti_aliasing AA)
{
    ...
    command_list->CopyBufferRegion(d3d12.const_buffer, 0, d3d12.upload_buffer, 0, sizeof(float) * 4 * 4);
    command_list->CopyBufferRegion(d3d12.vertex_buffer, 0, d3d12.upload_buffer, sizeof(float) * 4 * 4, d3d12.max_vertex_buffer);
    command_list->CopyBufferRegion(d3d12.index_buffer, 0, d3d12.upload_buffer, sizeof(float) * 4 * 4 + d3d12.max_vertex_buffer, d3d12.max_index_buffer);
...
nk_d3d12_init(ID3D12Device* device, int width, int height, unsigned int max_vertex_buffer, unsigned int max_index_buffer, unsigned int max_user_textures)
{
    ...
    //nk_buffer_init_fixed(&d3d12.cmds, nk_buffer_memory(&d3d12.ctx.memory), nk_buffer_total(&d3d12.ctx.memory));
    ...
    /* Create upload buffer */
    {
        ...
        hr = device->CreateCommittedResource(&d3d12.heap_prop_upload,
    ...
    /* Create constant buffer */
    {
        ...
        hr = device->CreateCommittedResource(&d3d12.heap_prop_default,
    ...
    d3d12.cbv_srv_uav_desc_increment = device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    ...
    hr = device->CreateRootSignature(0, nk_d3d12_vertex_shader,
        sizeof(nk_d3d12_vertex_shader), IID_PPV_ARGS(&d3d12.root_signature));
    NK_ASSERT(SUCCEEDED(hr));
    ...
    //    device->CreateRootSignature(0, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&d3d12.root_signature));
    //    blob->Release();
    //}
    ...
    /* Create pipeline state */
    {
        ...
        desc.DepthStencilState.FrontFace.StencilFailOp = desc.DepthStencilState.FrontFace.StencilDepthFailOp = desc.DepthStencilState.FrontFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;
        desc.DepthStencilState.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;
        desc.DepthStencilState.BackFace = desc.DepthStencilState.FrontFace;

        /* Create PSO */
        hr = device->CreateGraphicsPipelineState(&desc,
            IID_PPV_ARGS(&d3d12.pipeline_state));
        NK_ASSERT(SUCCEEDED(hr));
    }
...
nk_d3d12_font_stash_end(ID3D12GraphicsCommandList* command_list)
{
    ...
    /* Create font texture */
    {
        ...
        hr = d3d12.device->CreateCommittedResource(
            &d3d12.heap_prop_default, D3D12_HEAP_FLAG_NONE, &desc,
            D3D12_RESOURCE_STATE_COPY_DEST, NULL, IID_PPV_ARGS(&d3d12.font_texture));
    ...
    /* Create font upload buffer */
    {
        ...
        hr = d3d12.device->CreateCommittedResource(
            &d3d12.heap_prop_upload, D3D12_HEAP_FLAG_NONE, &desc,
            D3D12_RESOURCE_STATE_GENERIC_READ, NULL, IID_PPV_ARGS(&d3d12.font_upload_buffer));
...
void nk_d3d12_shutdown(void)
{
    ...
    /* DirectX 12 cleanup */
    if (d3d12.device) d3d12.device->Release();
    if (d3d12.pipeline_state) d3d12.pipeline_state->Release();
    if (d3d12.root_signature) d3d12.root_signature->Release();
    if (d3d12.desc_heap) d3d12.desc_heap->Release();
    if (d3d12.upload_buffer) d3d12.upload_buffer->Release();
    if (d3d12.const_buffer) d3d12.const_buffer->Release();
    if (d3d12.index_buffer) d3d12.index_buffer->Release();
    if (d3d12.vertex_buffer) d3d12.vertex_buffer->Release();
    if (d3d12.font_texture) d3d12.font_texture->Release();
    if (d3d12.font_upload_buffer) d3d12.font_upload_buffer->Release();
}
