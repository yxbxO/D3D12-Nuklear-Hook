...
/*
 * USAGE:
 *    - This function will initialize a new nuklear rendering context. The context will be bound to
 * a GLOBAL DirectX 12 rendering state.
 */
NK_API struct nk_context* nk_d3d12_init(ID3D12Device* device, int width, int height,
                                        unsigned int max_vertex_buffer,
                                        unsigned int max_index_buffer,
                                        unsigned int max_user_textures);
/*
 * USAGE:
 *    - A call to this function prepares the global nuklear d3d12 backend for receiving font
 * information’s. Use the obtained atlas pointer to load all required fonts and do all required font
 * setup.
 */
NK_API void nk_d3d12_font_stash_begin(struct nk_font_atlas** atlas);
/*
 * USAGE:
 *    - Call this function after a call to nk_d3d12_font_stash_begin(...) when all fonts have been
 * loaded and configured.
 *    - This function will place commands on the supplied ID3D12GraphicsCommandList.
 *    - This function will allocate temporary data that is required until the command list has
 * finish executing. The temporary data can be free by calling nk_d3d12_font_stash_cleanup(...)
 */
 ...
 *    - Only call this function after the command list used in the nk_d3d12_font_stash_begin(...)
...
/*
 * USAGE:
 *    - This function will setup the supplied texture (ID3D12Resource) for rendering custom images
 * using the supplied D3D12_SHADER_RESOURCE_VIEW_DESC.
 *    - This function may override any previous calls to nk_d3d12_set_user_texture(...) while using
 * the same index.
 *    - The returned handle can be used as texture handle to render custom images.
 *    - The caller must keep track of the state of the texture when it comes to rendering with
 * nk_d3d12_render(...).
 */
NK_API nk_bool nk_d3d12_set_user_texture(unsigned int index, ID3D12Resource* texture,
                                         const D3D12_SHADER_RESOURCE_VIEW_DESC* description,
                                         nk_handle* handle_out);
/*
 * USAGE:
 *    - This function should be called within the user window proc to allow nuklear to listen to
 * window events
 */
NK_API int nk_d3d12_handle_event(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam);
/*
 * USAGE:
 *    - A call to this function renders any previous placed nuklear draw calls and will flush all
 * nuklear buffers for the next frame
 *    - This function will place commands on the supplied ID3D12GraphicsCommandList.
 *    - When using custom images for rendering make sure they are in the correct resource state
 * (D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE) when calling this function.
...
NK_API void nk_d3d12_render(ID3D12GraphicsCommandList* command_list, enum nk_anti_aliasing AA) {
    ...
    // memset(&vertex_buffer_view, 0, sizeof(D3D12_VERTEX_BUFFER_VIEW));
    // memset(&index_buffer_view, 0, sizeof(D3D12_INDEX_BUFFER_VIEW));
    // memset(&map_range, 0, sizeof(D3D12_RANGE));
    // memset(resource_barriers, 0, sizeof(D3D12_RESOURCE_BARRIER) * 3);

    /* Activate D3D12 pipeline state and config root signature */
    command_list->SetPipelineState(d3d12.pipeline_state);
    command_list->SetGraphicsRootSignature(d3d12.root_signature);
    command_list->SetDescriptorHeaps(1, &d3d12.desc_heap);
    command_list->SetGraphicsRootDescriptorTable(0, d3d12.gpu_descriptor_handle);
    ...
    command_list->RSSetViewports(1, &d3d12.viewport);
    ...
    hr = d3d12.upload_buffer->Map(0, &map_range, (void**)&ptr_data);
    ...
    /* Nuklear convert and copy to upload buffer */
    {
        ...
        nk_buffer_init_fixed(&vbuf, &ptr_data[sizeof(float) * 4 * 4],
                             (size_t)d3d12.max_vertex_buffer);
        nk_buffer_init_fixed(&ibuf, &ptr_data[sizeof(float) * 4 * 4 + d3d12.max_vertex_buffer],
                             (size_t)d3d12.max_index_buffer);
        nk_convert(&d3d12.ctx, &d3d12.cmds, &vbuf, &ibuf, &config);
    }

    /* Close mapping range */
    d3d12.upload_buffer->Unmap(0, &map_range);
    ...
    command_list->ResourceBarrier(3, resource_barriers);

    /* Copy from upload buffer to gpu buffers */
    command_list->CopyBufferRegion(d3d12.const_buffer, 0, d3d12.upload_buffer, 0,
                                   sizeof(float) * 4 * 4);
    command_list->CopyBufferRegion(d3d12.vertex_buffer, 0, d3d12.upload_buffer,
                                   sizeof(float) * 4 * 4, d3d12.max_vertex_buffer);
    command_list->CopyBufferRegion(d3d12.index_buffer, 0, d3d12.upload_buffer,
                                   sizeof(float) * 4 * 4 + d3d12.max_vertex_buffer,
                                   d3d12.max_index_buffer);
    ...
    nk_draw_foreach(cmd, &d3d12.ctx, &d3d12.cmds) {
        ...
        if (cmd->elem_count) {
            ...
            command_list->SetGraphicsRoot32BitConstants(1, 1, &texture_id, 0);

            /* Dispatch draw call */
            command_list->DrawIndexedInstanced((UINT)cmd->elem_count, 1, offset, 0, 0);
...
static void nk_d3d12_get_projection_matrix(int width, int height, float* result) {
    const float L = 0.0f;
...
NK_API void nk_d3d12_resize(int width, int height) {
    ...
    /* Describe area to be mapped (the upload buffer region where the constant buffer / projection
     * matrix) lives */
    map_range.Begin = 0;
    map_range.End = sizeof(float) * 4 * 4;

    /* Map area to cpu accassible pointer (from upload buffer) */
    if (SUCCEEDED(d3d12.upload_buffer->Map(0, &map_range, &ptr_data))) {
...
// #pragma optimize( "", off )
NK_API int nk_d3d12_handle_event(HWND wnd, UINT msg, WPARAM wparam, LPARAM lparam) {
    switch (msg) {
        case WM_KEYDOWN:
        case WM_KEYUP:
        case WM_SYSKEYDOWN:
        ...
        case WM_LBUTTONDOWN:
            nk_input_button(&d3d12.ctx, NK_BUTTON_LEFT, (short)LOWORD(lparam),
                            (short)HIWORD(lparam), 1);
            SetCapture(wnd);
            return 1;

        case WM_LBUTTONUP:
            nk_input_button(&d3d12.ctx, NK_BUTTON_DOUBLE, (short)LOWORD(lparam),
        ...
        case WM_RBUTTONDOWN:
            nk_input_button(&d3d12.ctx, NK_BUTTON_RIGHT, (short)LOWORD(lparam),
                            (short)HIWORD(lparam), 1);
            SetCapture(wnd);
            return 1;
        ...
        case WM_MBUTTONDOWN:
            nk_input_button(&d3d12.ctx, NK_BUTTON_MIDDLE, (short)LOWORD(lparam),
                            (short)HIWORD(lparam), 1);
            SetCapture(wnd);
            return 1;
...
static void nk_d3d12_clipboard_paste(nk_handle usr, struct nk_text_edit* edit) {
    (void)usr;
    if (IsClipboardFormatAvailable(CF_UNICODETEXT) && OpenClipboard(NULL)) {
        ...
        if (mem) {
            ...
            if (size) {
                ...
                if (wstr) {
                    int utf8size = WideCharToMultiByte(CP_UTF8, 0, wstr, size / sizeof(wchar_t),
                                                       NULL, 0, NULL, NULL);
                    if (utf8size) {
                        char* utf8 = (char*)malloc(utf8size);
                        if (utf8) {
                            WideCharToMultiByte(CP_UTF8, 0, wstr, size / sizeof(wchar_t), utf8,
                                                utf8size, NULL, NULL);
...
static void nk_d3d12_clipboard_copy(nk_handle usr, const char* text, int len) {
    (void)usr;
...
NK_API struct nk_context* nk_d3d12_init(ID3D12Device* device, int width, int height,
    ...
    // nk_buffer_init_fixed(&d3d12.cmds, nk_buffer_memory(&d3d12.ctx.memory),
    // nk_buffer_total(&d3d12.ctx.memory));
    ...
    /* Create upload buffer */
    {
        ...
        desc.Width = (sizeof(float) * 4 * 4) + max_vertex_buffer +
                     max_index_buffer; /* Needs to hold matrix + vertices + indicies */
        desc.Height = 1;
        ...
        hr = device->CreateCommittedResource(&d3d12.heap_prop_upload, D3D12_HEAP_FLAG_NONE, &desc,
                                             D3D12_RESOURCE_STATE_GENERIC_READ, NULL,
                                             IID_PPV_ARGS(&d3d12.upload_buffer));
    ...
    /* Create constant buffer */
    {
        ...
        desc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
        desc.Alignment = D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT;
        desc.Width = 256; /* Should be sizeof(float) * 4 * 4 - but this does not match how d3d12
                             works (min CBV size of 256) */
        desc.Height = 1;
        desc.DepthOrArraySize = 1;
        desc.MipLevels = 1;
        desc.Format = DXGI_FORMAT_UNKNOWN;
        ...
        hr = device->CreateCommittedResource(&d3d12.heap_prop_default, D3D12_HEAP_FLAG_NONE, &desc,
                                             D3D12_RESOURCE_STATE_COMMON, NULL,
                                             IID_PPV_ARGS(&d3d12.const_buffer));
    ...
    /* Create vertex buffer */
    {
        ...
        hr = device->CreateCommittedResource(&d3d12.heap_prop_default, D3D12_HEAP_FLAG_NONE, &desc,
                                             D3D12_RESOURCE_STATE_COMMON, NULL,
                                             IID_PPV_ARGS(&d3d12.vertex_buffer));
    ...
    /* Create index buffer */
    {
        ...
        hr = device->CreateCommittedResource(&d3d12.heap_prop_default, D3D12_HEAP_FLAG_NONE, &desc,
                                             D3D12_RESOURCE_STATE_COMMON, NULL,
                                             IID_PPV_ARGS(&d3d12.index_buffer));
    ...
    /* Create descriptor heap for shader root signature */
    {
        ...
        hr = device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&d3d12.desc_heap));
    }

    /* Get address of first handle (CPU and GPU) */
    d3d12.cpu_descriptor_handle = d3d12.desc_heap->GetCPUDescriptorHandleForHeapStart();
    d3d12.gpu_descriptor_handle = d3d12.desc_heap->GetGPUDescriptorHandleForHeapStart();

    /* Get addresses of vertex & index buffers */
    d3d12.gpu_vertex_buffer_address = d3d12.vertex_buffer->GetGPUVirtualAddress();
    d3d12.gpu_index_buffer_address = d3d12.index_buffer->GetGPUVirtualAddress();

    /* Get handle increment */
    d3d12.cbv_srv_uav_desc_increment =
        device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

    /* Create view to constant buffer */
    cbv.BufferLocation = d3d12.const_buffer->GetGPUVirtualAddress();
    cbv.SizeInBytes = 256;
    cbv_handle = d3d12.cpu_descriptor_handle;
    device->CreateConstantBufferView(&cbv, cbv_handle);

    /* Create root signature */
    hr = device->CreateRootSignature(0, nk_d3d12_vertex_shader, sizeof(nk_d3d12_vertex_shader),
                                     IID_PPV_ARGS(&d3d12.root_signature));
    NK_ASSERT(SUCCEEDED(hr));
    ...
    //    ID3DBlob* blob = NULL;
    //    hr = D3D12SerializeRootSignature(&desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob, NULL);

    //    NK_ASSERT(SUCCEEDED(hr));

    //    device->CreateRootSignature(0, blob->GetBufferPointer(), blob->GetBufferSize(),
    //    IID_PPV_ARGS(&d3d12.root_signature)); blob->Release();
    //}
    ...
    /* Create pipeline state */
    {
        ...
        const D3D12_INPUT_ELEMENT_DESC layout[] = {
            {"POSITION", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0,
             NK_OFFSETOF(struct nk_d3d12_vertex, position),
             D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
            {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, NK_OFFSETOF(struct nk_d3d12_vertex, uv),
             D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
            {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, NK_OFFSETOF(struct nk_d3d12_vertex, col),
             D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
        };
        ...
        desc.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM; /* NOTE: When using HDR rendering you might
                                                            have a different framebuffer format */
        desc.SampleDesc.Count = 1;
        desc.SampleDesc.Quality = 0;
        desc.NodeMask = 0;
        ...
        /* Create PSO */
        hr = device->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(&d3d12.pipeline_state));
        NK_ASSERT(SUCCEEDED(hr));
    }
...
NK_API void nk_d3d12_font_stash_end(ID3D12GraphicsCommandList* command_list) {
    ...
    /* Create font texture */
    {
        ...
        hr = d3d12.device->CreateCommittedResource(&d3d12.heap_prop_default, D3D12_HEAP_FLAG_NONE,
                                                   &desc, D3D12_RESOURCE_STATE_COPY_DEST, NULL,
                                                   IID_PPV_ARGS(&d3d12.font_texture));
    ...
    /* Create font upload buffer */
    {
        ...
        hr = d3d12.device->CreateCommittedResource(&d3d12.heap_prop_upload, D3D12_HEAP_FLAG_NONE,
                                                   &desc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL,
                                                   IID_PPV_ARGS(&d3d12.font_upload_buffer));

        NK_ASSERT(SUCCEEDED(hr));
    }

    /* Copy image data to upload buffer */
    hr = d3d12.font_upload_buffer->Map(0, NULL, &ptr_data);
    NK_ASSERT(SUCCEEDED(hr));
    memcpy(ptr_data, image, w * h * 4);
    d3d12.font_upload_buffer->Unmap(0, NULL);
    ...
    cpy_box.right = w;
    cpy_box.front = 0;
    command_list->CopyTextureRegion(&cpy_dest, 0, 0, 0, &cpy_src, &cpy_box);
    ...
    d3d12.device->CreateShaderResourceView(d3d12.font_texture, &srv_desc, srv_handle);
...
void nk_d3d12_font_stash_cleanup() {
    if (d3d12.font_upload_buffer) {
        d3d12.font_upload_buffer->Release();
        d3d12.font_upload_buffer = NULL;
    }
}

NK_API
nk_bool nk_d3d12_set_user_texture(unsigned int index, ID3D12Resource* texture,
                                  const D3D12_SHADER_RESOURCE_VIEW_DESC* description,
                                  nk_handle* handle_out) {
    ...
    if (index < d3d12.max_user_textures) {
        ...
        srv_handle.ptr =
            d3d12.cpu_descriptor_handle.ptr + ((2 + index) * d3d12.cbv_srv_uav_desc_increment);

        /* Create SRV */
        d3d12.device->CreateShaderResourceView(texture, description, srv_handle);
...
void nk_d3d12_shutdown(void) {
    ...
    if (d3d12.pipeline_state)
        d3d12.pipeline_state->Release();
    if (d3d12.root_signature)
        d3d12.root_signature->Release();
    if (d3d12.desc_heap)
        d3d12.desc_heap->Release();
    if (d3d12.upload_buffer)
        d3d12.upload_buffer->Release();
    if (d3d12.const_buffer)
        d3d12.const_buffer->Release();
    if (d3d12.index_buffer)
        d3d12.index_buffer->Release();
    if (d3d12.vertex_buffer)
        d3d12.vertex_buffer->Release();
    if (d3d12.font_texture)
        d3d12.font_texture->Release();
    if (d3d12.font_upload_buffer)
        d3d12.font_upload_buffer->Release();
}
