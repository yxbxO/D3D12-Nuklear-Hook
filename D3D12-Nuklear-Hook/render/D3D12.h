...
public:
	...
	void set_swap_chain(IDXGISwapChain3* swap_chain) { m_swap_chain = swap_chain; }
	void set_command_queue(ID3D12CommandQueue* command_queue) { m_command_queue = command_queue; }
	
	// Resource access
	IDXGISwapChain3* swap_chain() const { return m_swap_chain; }
	    ID3D12CommandQueue* command_queue() const { return m_command_queue; }
    nk_context* nuklear_context() const { return m_nk_ctx; }
